...
exports.initiatePayment = functions.https.onRequest(async (req, res) => {
    try {
        // Get custom amount from request body or query parameters
        const customAmount = req.body.amount || req.query.amount || '1.00';

        // Generate a random order ID
        const randomOrderId = Math.floor(Math.random() * 1000000000).toString();
